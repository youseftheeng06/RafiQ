...
public class TaskService {
    ...
    public List<Task> getDashboardOverviewTasks() {
        return taskRepo.findAll();
    }
    ...
    public Task markTaskAsSeen(Long taskId, User currentUser) {
        if (currentUser == null) {
            throw new RuntimeException("Current user is required");
        }
        ...
        if (!existingTask.getAssignedTo().getId().equals(currentUser.getId())) {
            throw new RuntimeException("Only the assigned staff member can mark this task as read");
        }

        if (existingTask.isSeenByStaff()) {
            return existingTask;
        }

        existingTask.setSeenByStaff(true);
        Task savedTask = taskRepo.save(existingTask);
        notifyTaskSeen(savedTask, currentUser);
        return savedTask;
    }
    ...
    private void notifyTaskAssigned(Task task) {
        Notification notif = Notification.builder()
                ...
                .content("Task " + task.getTitle() + " was sent to you by " + task.getAssignedBy().getName())
                .type("TASK")
                .taskId(task.getId())
                .build();
        ...
        messagingTemplate.convertAndSend("/topic/events", Map.of(
                ...
                "taskId", task.getId(),
                "title", task.getTitle(),
                "assignedBy", task.getAssignedBy().getName(),
    ...
    private void notifyTaskSeen(Task task, User seenBy) {
        Notification notif = Notification.builder()
                .title("Task Read")
                .content(seenBy.getName() + " read task " + task.getTitle())
                .type("TASK")
                .taskId(task.getId())
                .build();
        ...
        messagingTemplate.convertAndSend("/topic/events", Map.of(
                ...
                "taskId", task.getId(),
                "title", task.getTitle(),
                "seenBy", seenBy.getName(),
                "recipientId", task.getAssignedBy().getId()
        ));
    }
